// Exemplo de página inicial do aplicativo web de nutrição TBCA
// Este é um exemplo de como seria a implementação em Next.js

import { useState, useEffect } from 'react';
import Head from 'next/head';
import Link from 'next/link';

// Componentes simulados (estes seriam implementados em arquivos separados)
...
async function fetchAlimentos(termo = '', categoriaId = null, pagina = 1) {
  const limit = 12;
  const skip = (pagina - 1) * limit;
  
  let url = `${API_URL}/alimentos?skip=${skip}&limit=${limit}`;
